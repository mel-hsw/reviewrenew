...
**Catalog:** run `python scripts/consolidate_products.py` to build `data/products.json` + `data/products_manifest.json` from slices in **`data/raw/amazon_reviews_2023/`**. Default ASINs: **Pet** `B0C4BFWLNC`, **Beauty** `B0BG9Q18ZZ`, **Home/Appliance** `B0BXSQN4HV`.
...
**Git:** `outputs/` and `data/chroma/` are **gitignored**. For submission, zip artifacts per course instructions—see [`submission/README.md`](submission/README.md).
...
├── .env.example
├── submission/
│   └── README.md              # Deliverables / grading checklist
├── data/
│   ├── README.md              # Raw vs derived vs chroma
│   ├── products.json
│   ├── products.example.json
│   ├── products_manifest.json # optional; from consolidate_products
│   ├── raw/
│   │   └── amazon_reviews_2023/   # category JSONL slices for Q1 consolidation
│   └── chroma/                # runtime (gitignored)
├── docs/
│   ├── agentic_workflow.md    # Q1–Q4, supervisor, harness (report-ready)
│   └── report_draft.md
├── outputs/                   # bundles, PNGs, run_log (gitignored)
├── scripts/
│   ├── consolidate_products.py
│   └── filter_meta_by_rating.py
└── src/
    ├── agent.py               # Supervisor
    ├── llm_harness.py         # Structured JSON + telemetry + retries
    ├── llm_analysis.py        # RAG + Q2/Q3 + merge
    ├── rag.py
    ├── chunking.py
    ├── image_gen.py
    ├── models.py
...
Experiments and rubric-aligned narrative: **[`docs/report_draft.md`](docs/report_draft.md)**.  
